...
8.3 비옥도 및 식생 순환 (Fertility & Growth)

대지는 양분을 소모하고 회복하는 살아있는 자원으로 취급됩니다.

*   **확산 (Propagation)**: 높은 비옥도를 가진 지점은 주변의 메마른 타일로 비옥도를 자연스럽게 전파(Diffusion)합니다.
*   **소모 (Consumption)**: 모든 식물(Grass 등)은 탄생 시 해당 지점의 비옥도를 일정량 소모합니다. 비옥도가 고갈되면 성장이 멈춥니다.
*   **시각적 품질 (Quality Mapping)**:
    *   탄생 지점의 비옥도에 따라 풀의 **색상과 영양가(Value)**가 결정됩니다.
    *   고비옥도: 진한 녹색, 높은 식량 가치 / 저비옥도: 시든 황갈색, 낮은 식량 가치.
*   **영양 사이클 (Nutrient Loop)**: 생물(양 등)이 풀을 뜯어 먹으면 풀의 비옥도가 개체 내부로 저장되며, 이후 **배설(Excretion)**을 통해 특정 지점에 농축된 비옥도를 환원합니다.
  * 틱당 위장 수치 감소 -> 배설물 수치 누적.
  * 한계치 도달 시 현재 좌표에 새로운 자원 노드(거름) 스폰.
  * 거름은 다시 식물 성장을 촉진하는 자원으로 순환.
