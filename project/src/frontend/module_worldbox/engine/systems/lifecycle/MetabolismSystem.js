...
export default class MetabolismSystem extends System {
    ...
    update(dt, time) {
        ...
        for (const id of em.animalIds) {
            // 🚀 [DOD] 버퍼에서 위치 및 시간 데이터 추출
            const x = bm.x[id];
            const y = bm.y[id];
            const lastUpdateTime = bm.lastUpdate[id];

            // 🚀 [Expert Optimization] 대사 LOD 적용
            const isVisible = camera && (x > viewX && x < viewX + viewW &&
                                        y > viewY && y < viewY + viewH);

            if (!isVisible) {
                const updateInterval = 1.0; // 1초
                if (time - lastUpdateTime < updateInterval * 1000) continue;

                const lodDt = (time - lastUpdateTime) / 1000;
                bm.lastUpdate[id] = time;
                this._updateMetabolism(id, lodDt, time);
            } else {
                bm.lastUpdate[id] = time;
                this._updateMetabolism(id, effectiveDt, time);
            }
        }
    ...
    _updateMetabolism(id, effectiveDt, time) {
        const em = this.entityManager;
        ...
        bm.hunger[id] = Math.max(0, bm.hunger[id] - hungerDecay);
        bm.fatigue[id] = Math.min(100, bm.fatigue[id] + fatigueIncrease);

        const currentHunger = bm.hunger[id];
        const currentFatigue = bm.fatigue[id];

        // 👴 2. 노화 처리
        if (age) {
            age.currentAge += effectiveDt * 0.0013889;
            if (Math.random() < 0.05) age.updateStage(entity);
            if (age.currentAge >= age.maxAge) bm.hp[id] = 0; // 자연사
        }

        // 💀 3. 아사 처리
        if (currentHunger <= 0) {
            bm.hp[id] -= effectiveDt * 2.0;
            if (bm.hp[id] <= 0) {
        ...
        // 👴 노화 사망 로그
        if (age && age.currentAge >= age.maxAge && bm.hp[id] <= 0) {
        ...
        // 💩 5. 배설 로직 (DOD와 기존 로직 브릿지)
        if (metabolism) {
            const transform = entity.components.get('Transform');
            if (transform) {
                metabolism.stomach = (currentHunger / 100) * metabolism.maxStomach;
                this.processInternalMetabolism(id, entity, animal, metabolism, transform, effectiveDt);
            }
        }

        // 🤕 7. 부상 회복 (DOD 최적화 필요 시 나중에 버퍼화)
